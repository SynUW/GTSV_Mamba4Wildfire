...
#             Hidden_State = torch.zeros(batch_size, self.feature_size, device=device)
#         if Cell_State is None:
#             Cell_State = torch.zeros(batch_size, self.feature_size, device=device)
#         if rHidden_State is None:
#             rHidden_State = torch.zeros(batch_size, self.feature_size, device=device)
#         if rCell_State is None:
#             rCell_State = torch.zeros(batch_size, self.feature_size, device=device)
...
#                        torch.mv(self.A_list[-1].to(device), self.Neighbor_weight))
#         Cell_State = f * NC + i * C
...
#         #return Hidden_State, Cell_State, gc, rHidden_State, rCell_State, pred
    ...
    #     rHidden_State = torch.zeros(batch_size, self.hidden_size, device=device)
    #     rCell_State = torch.zeros(batch_size, self.hidden_size, device=device)
    #     return Hidden_State, Cell_State, rHidden_State, rCell_State

    # def reinitHidden(self, batch_size, Hidden_State_data, Cell_State_data):
    #     device = next(self.parameters()).device
    #     Hidden_State = Hidden_State_data.to(device).requires_grad_(True)
    #     Cell_State = Cell_State_data.to(device).requires_grad_(True)
    #     rHidden_State = Hidden_State_data.to(device).requires_grad_(True)
    #     rCell_State = Cell_State_data.to(device).requires_grad_(True)
    #     return Hidden_State, Cell_State, rHidden_State, rCell_State

class KFGN(nn.Module):
    """
    KFGN（删掉 Kalman 融合版）：
    - 保留：DynamicFilter-GNN 多阶图卷积 + 图分支门控更新（f/i/o/C + Neighbor-based cell propagation）
    - 删除：rLSTM 分支 + var 融合 pred
    - 输出：Hidden_State（图分支的输出）
    """
    def __init__(self, K, A, feature_size, Clamp_A=True):
        super(KFGN, self).__init__()
        self.feature_size = feature_size
        ...
        # ---- normalize A (same as your code) ----
        D_inverse = torch.diag(1 / torch.sum(A, 0))
        norm_A = torch.matmul(D_inverse, A)
        A = norm_A

        # ---- build K-hop adjacency list (same as your code) ----
        A_temp = torch.eye(feature_size, feature_size)
        for _ in range(K):
            A_temp = torch.matmul(A_temp, A)
            if Clamp_A:
        ...
        # ---- DynamicFilterGNN per hop (same as your code) ----
        self.gc_list = nn.ModuleList([
            DynamicFilterGNN(feature_size, feature_size, self.A_list[i], bias=False)
            for i in range(K)
        ])

        hidden_size = self.feature_size
        gc_input_size = self.feature_size * K  # 与原代码保持一致（即使后面 reshape 很“特化”）

        # ---- graph-aware gated update (same as your code) ----
        self.fl = nn.Linear(gc_input_size + hidden_size, hidden_size)
        self.il = nn.Linear(gc_input_size + hidden_size, hidden_size)
        self.ol = nn.Linear(gc_input_size + hidden_size, hidden_size)
        self.Cl = nn.Linear(gc_input_size + hidden_size, hidden_size)

        # neighbor weighting (same as your code)
        self.Neighbor_weight = Parameter(torch.FloatTensor(feature_size))
        stdv = 1. / math.sqrt(feature_size)
        self.Neighbor_weight.data.uniform_(-stdv, stdv)
    ...
    def forward(self, input, Hidden_State=None, Cell_State=None):
        """
        input: [B, T, F]  （保持你原 forward 假设）
        return: Hidden_State (graph branch), shape 取决于你后续 reshape 设计
        """
        batch_size, time_steps, _ = input.shape
        device = input.device
        ...
        # ---- multi-hop dynamic filter graph conv (same as your code) ----
        gc = self.gc_list[0](x)
        for i in range(1, self.K):
            gc = torch.cat((gc, self.gc_list[i](x)), 1)

        # ---- gated update (same as your code) ----
        combined = torch.cat((gc, Hidden_State), 1)
        ...
        # neighbor-informed cell propagation (same as your code)
        NC = torch.mul(
            Cell_State,
            torch.mv(self.A_list[-1].to(device), self.Neighbor_weight)
        )
        ...
        # ---- 删除 Kalman 融合：直接返回图分支输出 ----
        return Hidden_State

    def loop(self, inputs):
        """
        inputs: [B, T, F]
        返回最后一步（或你 forward 输出定义下的最后一个时间片）
        """
        batch_size = inputs.size(0)
        time_step = inputs.size(1)
        ...
        out = None
        for i in range(time_step):
            out = self.forward(inputs[:, i:i + 1, :], Hidden_State, Cell_State)
            # 由于你 forward 里 expand 到 time_steps，这里 i:i+1 时 out 仍是长度 1 的序列
            # 取出状态用于下一步（保持与原思路一致）
            Hidden_State = out[:, -1, :]
            Cell_State = Cell_State[:, -1, :] if Cell_State.dim() == 3 else Cell_State
