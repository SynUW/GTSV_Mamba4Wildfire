...
try:
    from model_zoo.MambaHSI import BothMamba
except ImportError:
    from MambaHSI import BothMamba
...
def _mambahsi_group_num(channels: int) -> int:
    if channels % 38 == 0:
        return 38
    for g in (55, 37, 20, 19, 10, 8, 5, 4, 2, 1):
        if channels % g == 0:
            return g
    return 1


def _mambahsi_token_num(channels: int) -> int:
    for t in (10, 8, 5, 4, 2, 1):
        if channels % t == 0:
            return t
    return 8
...
class MambaModel(nn.Module):
    def __init__(self, num_classes=1, patch_size=13, num_bands=380, hidden_dim=128):
        ...
        group_num = _mambahsi_group_num(hidden_dim)
        token_num = _mambahsi_token_num(hidden_dim)
