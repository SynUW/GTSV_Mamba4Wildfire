"""
SSM_learnable_umap_3.py

================================================================
说明 / Note (中文)
================================================================
本文件是 SSM_learnable_umap_2.py 的派生版本，把 forecasting_pin/SSM_stable_v2
（即 forecasting_graph/SSM_t5）相对于 SSM_stable 的改动迁移到本网络的
SSM 骨干部分。UMAP head / MiniGCN / 可视化工具等非骨干模块**保持不变**。

【环境差异（与 SSM_t5 相比）】
  - 变量数 = 38（不是 55），seq_len=10，因此主干 num_bands = 380。
  - hidden_dim = 380；fc_out 使用 d_model（默认 128，外部 configs 可改）。
  - 所有 GroupNorm 的 requested_groups 用 38（不是 55），通过
    _safe_group_norm 自适应回退到能整除的最大约数。

【已迁移的 t5/v2 改动】
  1) DyT → RMSNorm
     - SparseDeformableMambaBlock.norm（原 self.norm = DyT(dim)）。
     - SparseDeformableChannelMambaBlock.norm（原 self.norm = DyT(dim)）。
  2) BatchNorm2d → _safe_group_norm(num_channels, 38) 在以下位置：
     - MambaBlock.norm。
     - SpatialSpectralMambaBlock.norm、其 self.conv 中的 BN2d、
       其 self.proj 中的 BN2d。
     - MambaModel.stem 中的 BN2d（被整体替换，见 (4)）。
     - MambaModel.conv 中的 BN2d。
  3) GELU → SiLU 在以下位置：
     - SpatialSpectralMambaBlock.conv、SpatialSpectralMambaBlock.proj。
     - MambaModel.stem（被整体替换，见 (4)）。
     - MambaModel.conv。
     - Model.fc_out。
  4) MambaModel.stem 整体替换为 t5 风格：
        Conv2d(num_bands, hidden_dim, kernel_size=1)
          -> _safe_group_norm(hidden_dim, 38)
          -> SiLU()
     原 stem 是 groups=38 的 3x3 + 1x1 分组卷积，会在入口处把
     38 组通道彼此隔离；新 stem 用 1x1 全混合，让 38×10=380 通道
     从第一层即可自由交互（v2 的核心 hypothesis）。
  5) Model.fc_out 中的 BatchNorm1d(d_model) → _safe_group_norm(d_model, 38)，
     激活 GELU → SiLU。GroupNorm 对 (B, C) 形状同样合法。
  6) 注入辅助函数 _safe_group_norm(num_channels, requested_groups)，
     当 num_channels 不能被 requested_groups 整除时自动回退到能整除的
     最大约数；保证 d_model=128（不被 38 整除）等情形不会崩。

【未迁移 / 暂不动的部分（留待后续 umap_4+ 评估）】
  - UMAPAnchorLaplacian 中的 LayerNorm + GELU（UMAP head 不在 t5 改造谱系内，
    保留以便对照实验）。
  - MiniGCNLayer / MiniGCNStream 中的 BatchNorm1d + GELU（图分支独立模块，
    不属于 SSM 骨干）。
  - SimplifiedMambaBlock 内部 RMSNorm（已经是 RMSNorm，无需改动）。
  - DropPath 比例、Dropout、d_state/d_conv/expand 等 hyperparam 全部沿用 umap_2。
  - forward 张量整形逻辑、UMAP/GCN/CrossAttention 调用顺序、
    LULC 嵌入、PositionalEncoder 等接口完全保留。

【后续可能的 umap_4+ 衍生方向（与 t6+ 谱系对齐）】
  - token-grouped 无损 spectral / 稠密时间维 mamba / 双向 sparse spatial
    / 可学习 deformable offsets / 调正则 / 2D sin-cos pos-enc
    / UMAP head 也换成 RMSNorm+SiLU。
================================================================
"""
...
def _safe_group_norm(num_channels: int, requested_groups: int) -> nn.GroupNorm:
    """t5/v2 风格的安全 GroupNorm：当 num_channels 不能被 requested_groups
    整除时，自动回退到 <= requested_groups 的最大约数（至少为 1）。"""
    if num_channels <= 0:
...
class MambaBlock(nn.Module):
    def __init__(self, dim: int):
        ...
        self.norm = _safe_group_norm(dim, 38)
...
class MambaModel(nn.Module):
    def __init__(self, num_classes=1, patch_size=13, num_bands=380, hidden_dim=128):
        ...
        self.stem = nn.Sequential(
            # t5-style stem: 1x1 conv full-mix + GroupNorm + SiLU
