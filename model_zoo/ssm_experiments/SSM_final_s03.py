...
# joblib 加载 UMAP 检查点时会 unpickle spectral_lib.*，须能从「forecasting/」根目录导入该包
...
class UMAPAnchorLaplacian(nn.Module):
    ...
    def __init__(
        ...
        # 允许从任意 cwd 运行：将相对路径解析到 forecasting/ 目录
        base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
        if umap_model_path and not os.path.isabs(umap_model_path):
            umap_model_path = os.path.join(base_dir, umap_model_path)
        if library_path and not os.path.isabs(library_path):
            library_path = os.path.join(base_dir, library_path)
...
class SpatialSpectralMambaBlock(nn.Module):
    def __init__(self, dim: int, patch_size: int, num_heads = 8):
        ...
        self.spatial_mamba = SparseDeformableMambaBlock(dim=dim, drop_rate=0.5, sparsity_ratio=0.3)
        self.spectral_mamba = SparseDeformableChannelMambaBlock(dim=13*13, drop_rate=0.5, sparsity_ratio=0.3)
        self.temporal_mamba = SparseDeformableChannelMambaBlock(dim=13*13, drop_rate=0.5, sparsity_ratio=0.3)
        self.norm = nn.BatchNorm2d(dim)
...
def _print_model_child_params(model: nn.Module) -> None:
    rows = [(name, _module_param_count(child)) for name, child in model.named_children()]
    for name, n in sorted(rows, key=lambda x: -x[1]):
        print(f"  {name:28s}  {n:>12,}")


def _effective_sparse_k(seq_len: int, sparsity_ratio: float) -> int:
    return max(1, int(seq_len * float(sparsity_ratio)))


def _print_mamba_core_sparse_routes(mamba_core: "MambaModel", H: int, W: int) -> None:
    """
    与 MambaModel.forward 中 reshape 一致，列出各 Sparse* 模块作用轴上的长度 L、稀疏率 rho、
    进入 Mamba/SSM-循环的有效长度 k（token 数）。参数量不随 rho 变；实测耗时随 rho 变。
    """
    hw = H * W
    specs = [
        ("temporal_mamba", "temporal_mamba", hw, SparseDeformableChannelMambaBlock),
        ("spectral_mamba", "spectral_mamba", 38, SparseDeformableChannelMambaBlock),
        ("spatial_mamba", "spatial_mamba", hw, SparseDeformableMambaBlock),
    ]
    for title, attr, L, cls in specs:
        seq = getattr(mamba_core, attr, None)
        if seq is None:
            continue
        print(f"  --- {title} (序列长度 L={L}, 空间 H×W={H}×{W}) ---")
        for i, layer in enumerate(seq):
            if not isinstance(layer, cls):
                continue
            rho = float(getattr(layer, "sparsity_ratio", 1.0))
            k = _effective_sparse_k(L, rho)
            note = ""
            if cls is SparseDeformableChannelMambaBlock:
                note = "；全长仍算 QK^T (O(L²·heads))，稀疏仅缩短后续 Mamba 子序列"
            print(f"    [{i}] rho={rho:g}  ->  k={k}  (k/L={k / max(L, 1):.4f}){note}")


def _cuda_forward_ms_per_iter(
    model: Model,
    B: int,
    T: int,
    C: int,
    ...
    """同步计时，单次 forward 平均耗时 (ms)。反映真实稀疏路径 + Mamba CUDA。"""
    x_enc = torch.randn(B, T, C, H, W, device=device)
    x_enc[:, :, -1, :, :] = torch.randint(1, 18, (B, T, H, W), device=device).float()
    x_mark_enc = torch.zeros(B, T, mark_f, device=device)
    x_mark_enc[:, :, -1] = doy_1d.unsqueeze(0).expand(B, -1)
    x_dec = torch.zeros(B, pred_len, C, H, W, device=device)
    x_mark_dec = torch.zeros(B, pred_len, mark_f, device=device)
    x_mark_dec[:, :, -1] = 0.5
    burn_history = torch.rand(B, H, W, Tb, device=device)

    def _run():
        with torch.no_grad():
            model(x_enc, x_mark_enc, x_dec, x_mark_dec, burn_history=burn_history)
    ...
    e1.record()
    torch.cuda.synchronize()
    return float(e0.elapsed_time(e1)) / float(n_iter)
...
if __name__ == "__main__":
    ...
    model = Model(configs).to(device).eval()
    n_param = sum(p.numel() for p in model.parameters())
    n_train = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print("========== 参数量（论文以 total 为准；稀疏率 rho 不改变权重个数）==========")
    print(f"parameters (total)     : {n_param:,}")
    print(f"parameters (trainable) : {n_train:,}")
    print("---- Model 一级子模块 (递归累计) ----")
    _print_model_child_params(model)

    sb = getattr(getattr(model, "model", None), "spectral_blocks", None)
    if sb is not None and _module_param_count(sb) > 0:
        print(
            f"---- 注意: MambaModel.forward 当前未调用 spectral_blocks，但其权重仍计入 total ----"
        )
        print(f"  spectral_blocks (unused in forward): {_module_param_count(sb):,} params")

    print("========== 稀疏路径：各阶段有效 token 数 k（随 rho 变；用于解释算力随 rho 变化的原因）==========")
    _print_mamba_core_sparse_routes(model.model, H, W)

    _doy = torch.linspace(0.0, 1.0, T, device=device)

    x_enc = torch.randn(batch_size, T, C, H, W, device=device)
    x_enc[:, :, -1, :, :] = torch.randint(1, 18, (batch_size, T, H, W), device=device).float()
    x_mark_enc = torch.zeros(batch_size, T, mark_f, device=device)
    x_mark_enc[:, :, -1] = _doy.unsqueeze(0).expand(batch_size, -1)
    x_dec = torch.zeros(batch_size, pred_len, C, H, W, device=device)
    x_mark_dec = torch.zeros(batch_size, pred_len, mark_f, device=device)
    x_mark_dec[:, :, -1] = 0.5
    burn_history = torch.rand(batch_size, H, W, Tb, device=device)

    out = model(x_enc, x_mark_enc, x_dec, x_mark_dec, burn_history=burn_history)
    print(f"forward output shape (batch={batch_size}): {tuple(out.shape)}")

    print("========== 计算开销（真实 GPU 路径：含 Mamba CUDA 与当前稀疏实现）==========")
    for B in (1, batch_size):
        ms = _cuda_forward_ms_per_iter(
            model, B, T, C, H, W, pred_len, mark_f, Tb, device, _doy,
            n_warmup=15, n_iter=50,
        )
        print(f"  CUDA forward: batch={B:3d}  mean {ms:.3f} ms/iter  (~{1000.0 / ms:.1f} iter/s)")

    print(
        "说明: 静态工具(thop)不体现 token 稀疏与 mamba 内核细节; "
        "论文写算力建议以「本段 ms/iter」或自行报告 GFLOPs 理论式+rho 为主。"
    )

    # 可视化：x_enc (B, T, C, H, W) 取第 2、3、4、5 通道，每通道一行；T 上随机长度 5 的时间窗口
    # x_enc = torch.randn(2, int(38*10), 38, 13, 13)
