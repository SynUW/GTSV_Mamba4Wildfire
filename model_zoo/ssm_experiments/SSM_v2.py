...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None, burn_history=None):
        ...
        # 若创建 adapter 时 config.seq_len=1（如 test_vis_opt 里 lookback_length=1）则这里 T_mark=1，与真实 seq_len 不一致会报错
        _, T_mark, _ = x_mark_enc.shape
        # 编码器时间信息：最后一维为 doy_norm [0,1]，转成 1..366 做 embedding，再对 T_enc 聚合为 (B, d_model)
        doy_norm = x_mark_enc[:, :, -1]  # (B, T_enc)
        doy_idx = (doy_norm * 365.0 + 1).long().clamp(1, 366)  # (B, T_enc)
        doy_emb = self.doy_embedding(doy_idx)  # (B, T_enc, d_model)
        doy_emb = doy_emb.mean(dim=1)  # (B, d_model)，整段编码器时间聚合
