...
class UMAPAnchorLaplacian(nn.Module):
    ...
    def forward(self, x):
        ...
        # 可选：将 A 可视化为热力图并在格子中标注数值，便于调试图结构
        if getattr(self, "visualize_adjacency", False):
            ...
            ax.set_title("UMAP Anchor Adjacency A (S @ S^T)")
            ax.set_xlabel("j")
            ax.set_ylabel("i")
            # 在每个格子中写上数值（保留 4 位小数），根据背景深浅自动选择黑/白文字
            Bn = A_np.shape[0]
            ...
            plt.tight_layout()
            plt.savefig("ssm_umap_anchor_A_heatmap.png", dpi=150, bbox_inches="tight")
            plt.close(fig)
