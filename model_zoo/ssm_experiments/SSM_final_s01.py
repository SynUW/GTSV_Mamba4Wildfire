...
class SpatialSpectralMambaBlock(nn.Module):
    def __init__(self, dim: int, patch_size: int, num_heads = 8):
        ...
        self.spatial_mamba = SparseDeformableMambaBlock(dim=dim, drop_rate=0.5, sparsity_ratio=0.1)
        self.spectral_mamba = SparseDeformableChannelMambaBlock(dim=13*13, drop_rate=0.5, sparsity_ratio=0.1)
        self.temporal_mamba = SparseDeformableChannelMambaBlock(dim=13*13, drop_rate=0.5, sparsity_ratio=0.1)
        self.norm = nn.BatchNorm2d(dim)
