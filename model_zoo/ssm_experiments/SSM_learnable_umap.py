...
class UMAPAnchorLaplacian(nn.Module):
    """
    基于 UMAP + Anchor 库的可学习自适应图拉普拉斯矩阵构建模块。

    设计思路：
        1. UMAP 投影（不可导）：将输入样本投影到低维语义空间。
        2. 可学习相似度（可导）：sample_proj / anchor_proj 在投影空间内
           计算样本与 anchor 的余弦相似度，替代原来的固定 RBF kernel。
        3. Anchor 中介建图：A = S_sparse @ S_sparse^T，
           两个样本通过共享 anchor 建立连接，保留 anchor 语义中介的核心思想。
        4. 自适应调制（可导，参考 AAMGCRN）：用 hidden_features 学习
           [B, B] 调制矩阵 W，通过 Hadamard 乘积对 UMAP anchor 图进行
           任务驱动的细化。W 的梯度路径最短，直接连通 pred_loss。
        5. Anchor 利用率正则：最大化 anchor 激活分布的熵，防止少数 anchor
           垄断 Top-K，保证 anchor_proj 参数得到均匀训练。

    梯度路径：
        路径一（任务驱动）：pred_loss → GCN → L → A_adapted → W → adapt_proj → hidden_features → Mamba
        路径二（语义驱动）：pred_loss → GCN → L → A_adapted → A_umap → S_sparse → S → sample_proj / anchor_proj

    参数：
        umap_model_path  : 预训练 UMAP 模型的 .joblib 文件路径。
        library_path     : 光谱库 .npz 文件路径。
                           旧格式包含 'library_embeddings' (K, T, c)；
                           新格式包含 'pos_library_embeddings' 和
                           'neg_library_embeddings'，自动合并。
        n_input_channels : 原始输入的波段数，默认 38。
        hidden_dim       : sample_proj / anchor_proj 的输出维度，默认 32。
        d_model          : hidden_features 的维度（来自主干网络），默认 380。
        adapt_dim        : adapt_proj 的中间维度，默认 32。
        k_anchors        : Softmax 稀疏化的温度分母，控制稀疏程度。
                           注意：现在使用 Softmax 软稀疏化，k_anchors 不再
                           代表硬截断数量，而是通过 temperature 控制。
        temperature      : Softmax 温度，越小越趋近硬 Top-K，默认 1.0。
    ...
    输入：
        ...
        temperature     : float (可选) 覆盖初始化时的 temperature，
                          训练时可做退火调度。

    输出：
        L          : Tensor [B, B]   归一化拉普拉斯矩阵，直接送入 GCN。
        graph_loss : Tensor (标量)   anchor 利用率正则损失。
        S_sparse   : Tensor [B, K]   软稀疏化后的 anchor 相似度（供调试）。
    """
    ...
    def __init__(
        ...
        # ==============================================================
        # 3. 可学习相似度投影头
        #    sample_proj : 样本 UMAP 嵌入  → 比较空间 [hidden_dim]
        #    anchor_proj : anchor 嵌入      → 比较空间 [hidden_dim]
        #    两者输入输出维度相同，余弦相似度才有意义
        # ==============================================================
        ...
        #    输入  : hidden_features [B, d_model]（来自主干网络）
        #    输出  : W [B, B] 调制矩阵（天然对称，值域 [0,1]）
        #
        #    实现方式：对称双线性
        ...
        #
        #    天然对称（内积对称），无需额外对称化操作。
        #    梯度路径最短：pred_loss → GCN → L → A → W → adapt_proj → hidden_features
        # ==============================================================
    ...
    def _compute_similarity(self, z: torch.Tensor) -> torch.Tensor:
        ...
        参数：
            z : [B, T_lib*c]  样本 UMAP 嵌入（不可导部分的输出）
        返回：
            S : [B, K]        余弦相似度，负值截断为 0，值域 [0, 1]
        """
        z_q = self.sample_proj(z)                                    # [B, hidden_dim]
        z_q = F.normalize(z_q, p=2, dim=-1)

        z_k = self.anchor_proj(self.anchors_raw)                     # [K, hidden_dim]
        z_k = F.normalize(z_k, p=2, dim=-1)

        S = torch.mm(z_q, z_k.T)                                    # [B, K]
        S = F.relu(S)                                                # 截断负相关
        return S

    def _softmax_sparsify(self, S: torch.Tensor, temperature: float) -> torch.Tensor:
        """
        Softmax 软稀疏化：替代硬 Top-K，全程可导。

        温度调度建议：
            训练初期 temperature=1.0 → 所有 anchor 均匀贡献（探索）
            训练后期 temperature→0.1 → 权重集中于最相似 anchor（收敛）

        参数：
            S           : [B, K]  原始余弦相似度
            temperature : float   控制分布的尖锐程度
        返回：
            S_soft : [B, K]  归一化后的软权重，行和为 1
        """
        return F.softmax(S / (temperature + 1e-6), dim=-1)

    def _compute_adapt_weights(self, hidden_features: torch.Tensor) -> torch.Tensor:
        """
        用主干网络的隐特征计算 [B, B] 自适应调制矩阵。
        ...
        天然对称，值域 [0, 1]，梯度路径直接连通 hidden_features。
        ...
        W = torch.mm(h_norm, h_norm.T)                              # [B, B]
        W = torch.sigmoid(W)                                        # 值域 [0, 1]
        return W
    ...
    def _anchor_usage_loss(self, S: torch.Tensor) -> torch.Tensor:
        ...
        目标：最大化 anchor 激活分布的熵，防止少数 anchor 垄断，
              保证 anchor_proj 的所有参数都能得到梯度更新。

        注意：使用稀疏化前的原始 S，而非 S_sparse，
              这样未被选中的 anchor 也能收到梯度信号。
        ...
        anchor_prob  = anchor_usage / (anchor_usage.sum() + 1e-6)   # [K] 归一化
    ...
    def forward(
        ...
        返回：
            ...
            graph_loss : Tensor (标量)   anchor 利用率正则损失
            S_sparse   : Tensor [B, K]   软稀疏化后的 anchor 权重（供调试）
        """
        ...
        # ----------------------------------------------------------
        # Step 2: 可学习相似度（可导）
        #   sample_proj / anchor_proj 将 UMAP 嵌入映射到比较空间
        # ----------------------------------------------------------
        ...
        # ----------------------------------------------------------
        # Step 4: 通过 anchor 中介构建邻接矩阵（UMAP 图骨架）
        #   A_umap[i,j] > 0  ⟺  样本 i 和 j 共享高权重 anchor
        # ----------------------------------------------------------
        A_umap = torch.mm(S_sparse, S_sparse.T)                     # [B, B]

        # ----------------------------------------------------------
        # Step 5: 自适应调制（可导，参考 AAMGCRN）
        #   W 由 hidden_features 驱动，梯度路径最短
        #   A_adapted = W ∘ A_umap（Hadamard）
        #   物理意义：在 UMAP 语义结构的基础上，
        #             用任务特征进一步强化或抑制某些连边
        # ----------------------------------------------------------
        W = self._compute_adapt_weights(hidden_features)             # [B, B]
        A = W * A_umap                                               # [B, B] Hadamard
