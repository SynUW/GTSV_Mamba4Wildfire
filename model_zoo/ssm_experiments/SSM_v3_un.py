"""
相较于v2主要修改了 manifold laplacian中使用余弦相似度计算相似度矩阵，而不是使用欧氏距离计算相似度矩阵
"""
...
class UMAPAnchorLaplacian(nn.Module):
    def __init__(self, umap_model_path, library_path, sigma=1.0, device='cuda'):
        ...
        self.C = 38  # 原始通道数，将在第一次 forward 时推断
        
        # 将 Anchors 注册为 Buffer (固定参数，不更新梯度，但随模型移动)
        self.register_buffer('anchors', torch.from_numpy(anchors_flat).float())
        self.visualize_adjacency = True  # 设为 True 时在 forward 中保存邻接矩阵 A 的热力图
        print(f"[Graph] Ready. Anchors shape: {self.anchors.shape} (K={K}, T={T_lib}, c={c})")

    def _plot_adjacency_heatmap(self, A_np, save_path='ssm_adjacency_heatmap.png'):
        """邻接矩阵 A [B, B] 热力图并保存，格内显示原始值（保留4位小数）。"""
        fig, ax = plt.subplots(figsize=(8, 6))
        im = ax.imshow(A_np, cmap='viridis', aspect='auto')
        plt.colorbar(im, ax=ax, label=r'$A_{i,j}$')
        ax.set_title('Adjacency Matrix A (S @ S^T)')
        ax.set_xlabel('j')
        ax.set_ylabel('i')
        B = A_np.shape[0]
    ...
    def forward(self, x):
        ...
        # # print(f'dist median: {torch.median(dist)}')
        ...
        # 1. 对特征和 Anchor 进行 L2 归一化 (投影到高维单位球面上)
        z_norm = F.normalize(z, p=2, dim=-1)
        anchors_norm = F.normalize(self.anchors, p=2, dim=-1)
        
        # 2. 计算余弦相似度 (矩阵乘法)
        # cos_sim 的值域是 [-1, 1]，值越大表示方向越一致
        cos_sim = torch.mm(z_norm, anchors_norm.t())
        
        # 3. 将余弦相似度转换为距离度量 (Cosine Distance = 1 - Cosine Similarity)
        # dist_cos 的值域是 [0, 2]，这样就完全摆脱了 50 维带来的距离膨胀
        dist_cos = 1.0 - cos_sim
        # print(f'dist_cos median: {torch.median(dist_cos)}')
        
        # 4. 计算 S 相似概率
        # 此时 sigma 可以安心地设为 0.1 ~ 0.5 左右的小数值
        S = torch.exp(-(dist_cos ** 2) / (self.sigma ** 2 + 1e-6))
        
        # ==========================================
        # 新增 Step 4.5: 对 S 进行 Top-K 强制稀疏化
        # ==========================================
        K_anchors = 4  # 假设每个样本最多只保留最像的 3 个 Anchor (你可以设为 3 到 5)
        
        # 找到每个样本最相似的 K 个 Anchor
        topk_vals, topk_inds = torch.topk(S, k=K_anchors, dim=1)
        
        # 创建一个和 S 形状一样但全为 0 的矩阵
        S_sparse = torch.zeros_like(S)
        
        # 把保留下来的 Top-K 相似度填回原来的位置，其余 97 个 Anchor 的相似度强制为 0
        S_sparse.scatter_(1, topk_inds, topk_vals)

        # ==========================================
        # Step 5: 构建邻接矩阵 A (使用稀疏化后的 S_sparse)
        # ==========================================
        # A = S_sparse * S_sparse^T
        A = torch.mm(S_sparse, S_sparse.t())
        ...
        # A = torch.mm(S, S.t())
        # 可视化邻接矩阵
        # if getattr(self, 'visualize_adjacency', False):
        #     self._plot_adjacency_heatmap(A.detach().cpu().numpy())
...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None, burn_history=None):
        ...
        out2 = self.gcn_stream_manifold(x_enc[:, :, H//2, W//2], laplacian_manifold)  # (B, d_model)
        # out = self.fc_out(torch.cat([out, hidden_features, out2], dim=1))
        out = self.fc_out(torch.cat([out2, hidden_features], dim=1))
