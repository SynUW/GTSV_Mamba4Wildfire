...
在UMAP2的基础上对burn history改进，同样使其可学习
"""
...
"""
BurnHistoryLaplacian：为 construct_batch_laplacian 添加可学习投影降维。

设计思路与 UMAPAnchorLaplacian 保持一致，但针对燃烧历史的特殊性做了调整：

1. 问题分析
   - burn_history 经过 window-sum 压缩后形状为 [B, T//ws + r]，典型 T=365, ws=60 → ~6 维，
     但也可能直接是 [B, T_raw] 的稀疏长序列（大量 0、少量非零事件）。
   - 稀疏 + 长序列 → Jaccard 计算本身是 O(B²T) 的，T 大时效率低；
     更重要的是，Jaccard 只测量"事件重叠"，无法捕捉时序模式（哪个时期着火、
     着火强度分布形状等语义信息）。
   - 可学习投影将 [B, T] → [B, proj_dim]，让模型学会提取燃烧事件的语义特征，
     在低维空间做相似度度量，效果等价于"学习一个针对火灾历史的核函数"。

2. 相似度度量的修改
   加上可学习投影之后，原始 Jaccard 应当被替换/增强：

   A) 投影后的相似度度量：余弦相似度（推荐）
      理由：
        - 投影输出是 Dense 向量，Jaccard 的稀疏假设不再成立。
        - 余弦相似度对向量长度不敏感，与 L2-normalize + 内积等价，
          梯度流畅，与 UMAPAnchorLaplacian 的设计一致。
        - 原始 Jaccard 可作为辅助项保留（可选 alpha 混合），
          提供"事件重叠"的归纳偏置，防止模型完全遗忘物理语义。

   B) 投影架构选择：
        - 门控 1D-Conv + Linear：提取局部时序模式（连续燃烧事件），
          比纯 Linear 更适合长稀疏序列。
        - LayerNorm 稳定训练，GELU 非线性增强表达。
        - 输出做 L2 normalize，确保余弦相似度值域 [-1, 1]（clamp 到 [0, 1]）。

3. 结构
   BurnHistoryLaplacian(nn.Module)
     ├── temporal_proj  : [B, T] → [B, proj_dim]  （门控 Conv1d + Linear）
     ├── adapt_proj     : [B, d_model] → [B, adapt_dim]  （任务驱动调制，与 UMAP 分支对齐）
     └── forward(burn_history, hidden_features=None)
           → L [B, B], graph_loss (标量)
...
4. 梯度路径
   ...
   路径二（燃烧语义）：
       pred_loss → GCN → L → A → A_proj → S_proj → temporal_proj → burn_history

5. 与 Model.forward 的集成
   将原来的函数调用：
       laplacian = construct_batch_laplacian(burn_history)
   替换为模块调用：
       laplacian, burn_graph_loss = self.burn_laplacian(burn_history, hidden_features)
   并将 burn_graph_loss 加入总损失。
"""

class BurnHistoryLaplacian(nn.Module):
    """
    为历史燃烧数据构建可学习图拉普拉斯矩阵。

    核心改进（相较于纯函数 construct_batch_laplacian）：
      1. 可学习时序投影：[B, T] → [B, proj_dim]
         门控 Conv1d 提取局部燃烧事件模式，Linear 映射到低维语义空间。
      2. 投影空间余弦相似度替代原始 Jaccard（见下方设计说明）。
      3. 可选 alpha 混合：alpha * 投影余弦 + (1-alpha) * Jaccard 物理先验。
      4. 可选自适应调制（需传入 hidden_features）：与 UMAPAnchorLaplacian 对齐。

    相似度度量说明
    ──────────────
    原始 Jaccard：适合稀疏二值/连续非负向量，物理意义直观（事件重叠率）。
    投影后余弦：  在 Dense 低维空间度量方向相似性，可学习，梯度流畅。
    两者互补：
      - alpha=1.0：纯投影余弦，完全可学习，适合数据量大、模式复杂的场景。
      - alpha=0.0：纯 Jaccard，无可学习参数，退化为原始行为（用于消融）。
      - alpha=0.7（默认）：兼顾可学习性与物理先验，推荐起始值。

    参数
    ──────────────
    in_len       : burn_history 压缩后的时间长度 T（动态推断，此处为上界/初始值）。
    proj_dim     : 投影后的低维维度，默认 32（与 UMAPAnchorLaplacian.hidden_dim 对齐）。
    d_model      : hidden_features 的维度（主干网络输出），默认 380。
    adapt_dim    : 自适应调制的中间维度，默认 32。
    alpha        : 投影余弦与 Jaccard 的混合权重（0→纯 Jaccard, 1→纯余弦）。
    use_adapt    : 是否使用 hidden_features 做自适应调制，默认 True。
    top_k_ratio  : Top-K 稀疏化比例（相对于 B），默认 0.5（保留 B//2 条边）。
    lambda_reg   : 投影参数的 L2 正则权重，默认 1e-4。
    device       : 运行设备。
    """

    def __init__(
        self,
        in_len: int = 7,          # 压缩后 T，动态推断时可传 1（forward 内自适应）
        proj_dim: int = 32,
        d_model: int = 380,
        adapt_dim: int = 32,
        alpha: float = 0.7,       # 投影余弦权重；(1-alpha) 为 Jaccard 权重
        use_adapt: bool = True,   # 是否使用 hidden_features 自适应调制
        top_k_ratio: float = 0.5, # Top-K 稀疏化：保留 ceil(B * top_k_ratio) 条边
        ...
        # ──────────────────────────────────────────────────────────────
        # 1. 时序投影网络（门控 Conv1d + Linear）
        #    输入：[B, 1, T]（将 T 视为 1D 序列，C=1）
        #    Conv1d 提取局部时序模式（燃烧事件的形状、位置）
        #    Linear 映射到低维语义空间
        # ──────────────────────────────────────────────────────────────
        #
        # 设计说明：为何用 Conv1d 而非纯 Linear？
        #   燃烧历史是稀疏时序事件：某段连续时间着火，其余全 0。
        #   Conv1d 的局部感受野天然匹配"事件检测"的归纳偏置，
        #   比 Linear（全局权重）对平移不变性更友好。
        #   门控（sigmoid * tanh）进一步控制哪些局部模式被激活，
        #   类似 LSTM/GRU 的门机制，适合处理稀疏信号。
        #
        self.conv_gate = nn.Conv1d(1, proj_dim, kernel_size=3, padding='same', bias=False)
        self.conv_feat = nn.Conv1d(1, proj_dim, kernel_size=3, padding='same', bias=False)
        # 汇聚后的线性映射
        ...
        # ──────────────────────────────────────────────────────────────
        # 2. 自适应调制（同 UMAPAnchorLaplacian，参考 AAMGCRN）
        #    W = sigmoid(normalize(h) @ normalize(h)^T)
        #    梯度路径：pred_loss → GCN → L → A → W → adapt_proj → hidden_features
        # ──────────────────────────────────────────────────────────────
        if use_adapt:
    ...
    def _project(self, bh: torch.Tensor) -> torch.Tensor:
        B, T = bh.shape
        x = bh.unsqueeze(1)                            # [B, 1, T]
        gate = torch.sigmoid(self.conv_gate(x))         # [B, proj_dim, T]
        feat = torch.tanh(self.conv_feat(x))            # [B, proj_dim, T]
        out_conv = gate * feat                          # [B, proj_dim, T]

        # 用燃烧强度作为注意力权重，让有火的时间步主导池化
        # bh: [B, T] -> [B, 1, T]，normalize 后作为权重
        attn = bh.unsqueeze(1) + 1e-6                  # [B, 1, T]，避免全零
        attn = attn / attn.sum(dim=-1, keepdim=True)   # [B, 1, T]，归一化为概率
        out = (out_conv * attn).sum(dim=-1)             # [B, proj_dim]，加权求和

        out = self.proj_linear(out)
        return F.normalize(out, p=2, dim=-1)

    @staticmethod
    def _jaccard_similarity(bh: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
        """
        广义 Jaccard (Tanimoto) 相似度：[B, T] → [B, B]
        与原始 construct_batch_laplacian 完全一致，作为物理先验保留。
        """
        dot = torch.mm(bh, bh.t())                    # [B, B]
        norm_sq = (bh ** 2).sum(dim=1, keepdim=True)  # [B, 1]
        union = norm_sq + norm_sq.t() - dot            # [B, B]
        return dot / (union + eps)                     # [B, B]，值域 [0, 1]

    def _compute_adapt_weights(self, hidden_features: torch.Tensor) -> torch.Tensor:
        """同 UMAPAnchorLaplacian._compute_adapt_weights"""
        h = self.adapt_proj(hidden_features)           # [B, adapt_dim]
        h_norm = F.normalize(h, p=2, dim=-1)           # [B, adapt_dim]
        return torch.sigmoid(torch.mm(h_norm, h_norm.t()))  # [B, B]

    @staticmethod
    def _normalize_laplacian(A: torch.Tensor) -> torch.Tensor:
        """对称归一化拉普拉斯：L = D^{-1/2} (A+I) D^{-1/2}"""
        B = A.shape[0]
        ...
        return torch.mm(torch.mm(D_mat, A_hat), D_mat)

    def _reg_loss(self) -> torch.Tensor:
        """
        投影参数 L2 正则。
        防止 conv_gate / conv_feat / proj_linear 权重爆炸，
        类似 UMAPAnchorLaplacian 中 anchor_usage_loss 的 l2_reg 项。
        """
        loss = torch.tensor(0., device=next(self.parameters()).device)
    ...
    def forward(
        self,
        burn_history: Optional[torch.Tensor],
        hidden_features: Optional[torch.Tensor] = None,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        参数
        ─────
        burn_history    : [B, T] 压缩后的燃烧历史（非负连续值）。
                          None 时返回单位拉普拉斯（全连接均匀图）。
        hidden_features : [B, d_model] 主干网络输出（用于自适应调制）。
                          None 或 use_adapt=False 时跳过调制。

        返回
        ─────
        L          : [B, B] 归一化拉普拉斯矩阵。
        graph_loss : 标量，投影参数 L2 正则损失。
        """
        # ── 退化情况：无燃烧历史 ──────────────────────────────────────
        if burn_history is None:
            B = hidden_features.shape[0] if hidden_features is not None else 1
            device = (hidden_features.device
                      if hidden_features is not None
                      else next(self.parameters()).device)
            L = self._normalize_laplacian(torch.zeros(B, B, device=device))
            return L, torch.tensor(0., device=device)
        ...
        # ── Step 1：可学习投影 → 余弦相似度 ──────────────────────────
        #
        # 投影后的 z 是 L2-normalized [B, proj_dim]
        # S_proj[i,j] = z_i · z_j ∈ [-1, 1]
        # 截断到 [0, 1]（负相关不建立边）
        #
        z = self._project(bh)                          # [B, proj_dim]
        S_proj = F.relu(torch.mm(z, z.t()))            # [B, B]，余弦相似度，值域 [0,1]

        # ── Step 2：物理先验 Jaccard（可选混合）──────────────────────
        #
        # alpha 控制两种相似度的混合比例：
        #   alpha=1.0 → 完全依赖可学习投影（高数据量场景推荐）
        #   alpha=0.0 → 退化为原始 Jaccard（消融基线）
        #   alpha=0.7 → 默认，兼顾语义和物理先验
        #
        if self.alpha < 1.0:
            S_jaccard = self._jaccard_similarity(bh)   # [B, B]，值域 [0,1]
            S = self.alpha * S_proj + (1.0 - self.alpha) * S_jaccard
        else:
            S = S_proj

        # ── Step 3：Top-K 稀疏化（去掉自环对角，避免自比较）──────────
        #
        # 设计说明：为何仍保留 Top-K？
        #   投影后的相似度矩阵是 Dense 的，直接建全连接图会引入大量噪声边。
        #   Top-K 保留语义上最相似的 k 个邻居，与 UMAPAnchorLaplacian 的
        #   Softmax 稀疏化在功能上互补：UMAP 分支用 Softmax 软稀疏化（全局），
        #   本分支用 Top-K 硬稀疏化（局部邻居），两者并行更鲁棒。
        #
        k = max(1, min(int(B * self.top_k_ratio), B - 1))
        S_no_self = S.clone()
        S_no_self.fill_diagonal_(float('-inf'))
        vals, inds = torch.topk(S_no_self, k=k, dim=1)
        A = torch.zeros_like(S)
        A.scatter_(1, inds, vals.clamp_min(0.0))
        A = (A + A.t()) / 2.0                          # 对称化 → 无向图

        # ── Step 4：自适应调制（Hadamard 乘积，同 UMAPAnchorLaplacian）──
        if self.use_adapt and hidden_features is not None:
        ...
        # ── Step 6：正则损失 ──────────────────────────────────────────
        graph_loss = self._reg_loss()

        return L, graph_loss


# =============================================================================
# construct_batch_laplacian 保留为原始纯函数（向后兼容 + 消融基线）
# 新建 construct_batch_laplacian_learned 作为模块化入口（推荐用 BurnHistoryLaplacian）
# =============================================================================

def construct_batch_laplacian(burn_history, k=16*16, visualize=False,
                               visualize_path_prefix="ssm_batch_laplacian"):
    """
    原始 Jaccard 拉普拉斯（纯函数，无可学习参数）。
    保留用于消融实验对比。
    """
...
# =============================================================================
# Model 的修改部分（diff 形式，方便对照原始代码）
# =============================================================================
"""
在 Model.__init__ 中添加（紧跟 self.graph_layer 之后）：

    self.burn_laplacian = BurnHistoryLaplacian(
        proj_dim=32,
        d_model=d_model,       # 380
        adapt_dim=32,
        alpha=0.7,             # 投影余弦权重，可调
        use_adapt=True,        # 使用 hidden_features 调制
        top_k_ratio=0.5,
        lambda_reg=1e-4,
    )

在 Model.forward 中替换（原来的 laplacian = construct_batch_laplacian(burn_history)）：
    ...
    if burn_history is not None:
        ...
        laplacian = self._fallback_laplacian(hidden_features)   # 见下方
        burn_graph_loss = torch.tensor(0., device=hidden_features.device)

在 Model.forward 返回之前（或训练循环中）将 burn_graph_loss 加入总损失：
    total_loss = pred_loss + graph_loss + burn_graph_loss
    
_fallback_laplacian（当 burn_history=None 时）：
    用单位矩阵（每个节点只有自环，等价于无 GCN 信息传播）
    
    def _fallback_laplacian(self, hidden_features):
        B = hidden_features.shape[0]
        return torch.eye(B, device=hidden_features.device)
"""
...
class PositionalEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        x = Variable(self.pe[x[:, -1] - 1, :], requires_grad=False).to(self.device)
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.PositionalEncoding = PositionalEncoder(d_model, 366, 'cuda').to('cuda')
        ...
        self.burn_laplacian = BurnHistoryLaplacian(
            proj_dim=64,        # T=128，20年跨度，需要更多容量捕捉年际模式
            d_model=d_model,    # 380
            adapt_dim=32,       # 与 UMAPAnchorLaplacian 对齐
            alpha=0.8,          # window-sum 后稀疏性降低，提高投影权重
            use_adapt=True,
            top_k_ratio=0.3,    # batch 内大多数样本火灾模式差异大，图宜稀疏
            lambda_reg=2e-4,    # proj_dim 翻倍，正则适当加强
        )
