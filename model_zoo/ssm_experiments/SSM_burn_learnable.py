...
class UMAPAnchorLaplacian(nn.Module):
    """
    基于 UMAP + Anchor 库的可学习图拉普拉斯矩阵构建模块。

    核心设计思路：
        1. 将输入样本通过预训练 UMAP 模型投影到低维嵌入空间。
        2. 用可学习的投影头（sample_proj / anchor_proj）将样本嵌入和
           anchor 库嵌入映射到同一比较空间。
        3. 计算每个样本与所有 anchor 的余弦相似度，Top-K 稀疏化后，
           通过 A = S_sparse @ S_sparse^T 用 anchor 作为语义中介建图。
        4. 加 anchor 利用率正则，缓解 Top-K 截断导致的 anchor 参数
           长期得不到梯度更新的问题。

    参数：
        umap_model_path : str
            预训练 UMAP 模型的 .joblib 文件路径。
        library_path : str
            光谱库 .npz 文件路径，支持两种格式：
            - 旧格式：包含 'library_embeddings' (K, T, c)
            - 新格式：包含 'pos_library_embeddings' 和
                      'neg_library_embeddings'，自动合并
        n_input_channels : int
            原始输入的通道数（波段数），默认 38。
        hidden_dim : int
            sample_proj 和 anchor_proj 的输出维度，默认 32。
        k_anchors : int
            每个样本保留的 Top-K anchor 数量，默认 5。
        lambda_anchor : float
            anchor 利用率正则的损失权重，默认 0.01。
        device : str
            运行设备，默认 'cuda'。

    输入：
        x : Tensor [B, C*T]
            Flatten 后的时序光谱数据，C 为通道数，T 为时序长度。

    输出：
        L : Tensor [B, B]
            归一化拉普拉斯矩阵，可直接送入 GCN。
        graph_loss : Tensor (标量)
            anchor 利用率正则损失，需加入总 loss 一同优化。
        S_sparse : Tensor [B, K]
            稀疏化后的 anchor 相似度矩阵，供外部可视化或调试使用。
    """
    ...
    def __init__(
        ...
        if 'library_embeddings' in lib_data:
            ...
            anchors_np = np.concatenate([pos, neg], axis=0)     # (K_pos+K_neg, T, c)
        else:
            raise KeyError(
        ...
        # ==============================================================
        # 3. 可学习投影头
        #    sample_proj : 样本 UMAP 嵌入 → 比较空间
        #    anchor_proj : anchor 嵌入     → 比较空间（同一空间）
        #    两者共用相同的输入/输出维度，保证余弦相似度有意义
        # ==============================================================
    ...
    def _umap_transform(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # flatten for UMAP: [B*T_lib, C]
        x_flat = x_last.reshape(-1, self.C)

        # UMAP transform（CPU，不可导）
        with torch.no_grad():
        ...
        # reshape: [B, T_lib*c]
        z = z_flat.view(B, self.T_lib * self.n_components)
        return z

    def _compute_similarity(self, z: torch.Tensor) -> torch.Tensor:
        """
        在可学习投影空间内计算样本与 anchor 的余弦相似度。

        参数：
            z : [B, T_lib*c]  样本 UMAP 嵌入
        返回：
            S : [B, K]        归一化余弦相似度，值域 [0, 1]
        """
        # 样本投影：[B, hidden_dim]
        z_q = self.sample_proj(z)
        z_q = F.normalize(z_q, p=2, dim=-1)

        # anchor 投影：[K, hidden_dim]
        z_k = self.anchor_proj(self.anchors_raw)
        z_k = F.normalize(z_k, p=2, dim=-1)

        # 余弦相似度：[B, K]，截断负值（负相关不构成连接）
        S = torch.mm(z_q, z_k.T)
        S = F.relu(S)
        return S

    @staticmethod
    def _topk_sparsify(S: torch.Tensor, k: int, temperature: float = 1.0) -> torch.Tensor:
        """
        用 Softmax 软稀疏化替代硬 Top-K。
        全程可导，梯度可以流回 sample_proj 和 anchor_proj。
        
        temperature 控制稀疏程度：
            → 1.0 : 较软，所有 anchor 都有贡献（训练初期）
            → 0.1 : 趋硬，权重集中在最相似的 anchor（训练后期）
        """
        return F.softmax(S / temperature, dim=-1)  # [B, K]
    ...
    def _anchor_usage_loss(self, S: torch.Tensor) -> torch.Tensor:
        ...
        目标：让所有 anchor 被尽量均匀地激活，防止少数 anchor 垄断
              Top-K 选择，导致 anchor_proj 中大部分参数长期无梯度。

        做法：最大化 anchor 激活分布的熵（等价于最小化负熵）。
        ...
        返回：
            loss : 标量
        """
        # 每个 anchor 在 batch 内的总激活量
        anchor_usage = S.sum(dim=0)                              # [K]
        # 归一化为概率分布
        anchor_prob = anchor_usage / (anchor_usage.sum() + 1e-6)
        # 负熵（最小化 → 最大化熵 → 均匀分布）
        neg_entropy = (anchor_prob * torch.log(anchor_prob + 1e-6)).sum()
        return neg_entropy
    ...
    def forward(self, x: torch.Tensor):
        ...
        返回：
            ...
            S_sparse   : Tensor [B, K]   稀疏化后的 anchor 相似度（供调试）
        """
        # ----------------------------------------------------------
        # Step 1: UMAP 投影（不可导）
        # ----------------------------------------------------------
        ...
        # ----------------------------------------------------------
        # Step 2: 可学习投影空间内计算样本-anchor 相似度（可导）
        # ----------------------------------------------------------
        S = self._compute_similarity(z)                          # [B, K]

        # ----------------------------------------------------------
        # Step 3: Top-K 稀疏化
        # ----------------------------------------------------------
        S_sparse = self._topk_sparsify(S, self.k_anchors)       # [B, K]

        # ----------------------------------------------------------
        # Step 4: 通过 anchor 中介构建邻接矩阵
        #   A[i,j] > 0  ⟺  样本 i 和 j 共享至少一个高权重 anchor
        # ----------------------------------------------------------
        A = torch.mm(S_sparse, S_sparse.T)                      # [B, B]
        ...
        # ----------------------------------------------------------
        # Step 6: Anchor 利用率正则损失
        #   注意：使用稀疏化前的 S（而非 S_sparse），
        #   这样梯度可以流向所有 anchor，而不只是被选中的 Top-K
        # ----------------------------------------------------------
        graph_loss = self.lambda_anchor * self._anchor_usage_loss(S)

        return L, graph_loss, S_sparse
...
class BurnHistoryLaplacian(nn.Module):
    """
    对 burn history 先降维再建图的可学习拉普拉斯矩阵。
    
    输入: burn_history [B, T_total]  原始聚合后序列（约120维）
    输出: L [B, B]                   归一化拉普拉斯
          graph_loss                  正则损失
    """
    def __init__(
        self,
        input_dim: int,        # 聚合后的序列长度，约120
        hidden_dim: int = 32,  # 降维目标维度
        k: int = 5,            # kNN 近邻数
        use_conv: bool = True, # True=Conv1d降维, False=Linear降维
        ...
        if use_conv:
            # Conv1d 降维：捕捉局部时间模式
            # 输入 [B, 1, T] → 输出 [B, hidden_dim]
            self.encoder = nn.Sequential(
                nn.Conv1d(1, 16, kernel_size=7, stride=2, padding=3),
                nn.GELU(),
                nn.Conv1d(16, hidden_dim, kernel_size=5, stride=2, padding=2),
                nn.GELU(),
                nn.AdaptiveAvgPool1d(1),   # 全局聚合 → [B, hidden_dim, 1]
    ...
    def forward(self, burn_history: torch.Tensor):
        """
        burn_history: [B, T_total]
        """
        B = burn_history.shape[0]

        # ----------------------------------------------------------
        # Step 1: 降维
        # ----------------------------------------------------------
        if self.use_conv:
            x = burn_history.unsqueeze(1)          # [B, 1, T]
            h = self.encoder(x)                    # [B, hidden_dim]
        else:
            h = self.encoder(burn_history.float()) # [B, hidden_dim]

        # ----------------------------------------------------------
        # Step 2: 余弦相似度建图（在低维空间）
        # ----------------------------------------------------------
        h_norm = F.normalize(h, p=2, dim=-1)       # [B, hidden_dim]
        S = torch.mm(h_norm, h_norm.T)             # [B, B]
        S = F.relu(S)                              # 只保留正相关

        # ----------------------------------------------------------
        # Step 3: Top-K 稀疏化（去掉自身后选K个邻居）
        # ----------------------------------------------------------
        S_no_self = S.clone()
        S_no_self.fill_diagonal_(float('-inf'))
        vals, inds = torch.topk(S_no_self, k=min(self.k, B-1), dim=1)
        ...
        # Step 4: 归一化拉普拉斯
        # ----------------------------------------------------------
        I = torch.eye(B, device=burn_history.device)
        A_hat = A + I
        D = A_hat.sum(dim=1)
        D_inv_sqrt = torch.pow(D, -0.5)
        D_inv_sqrt[torch.isinf(D_inv_sqrt)] = 0.
        ...
        # Step 5: 正则损失（防止图退化）
        # ----------------------------------------------------------
        mask = 1 - torch.eye(B, device=burn_history.device)
        sparsity_loss = (A * mask).mean()
        degree = (A * mask).sum(dim=-1)
        connectivity_loss = F.relu(1.0 - degree).mean()
        graph_loss = self.lambda_reg * (sparsity_loss + connectivity_loss)
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.burn_history_laplacian = BurnHistoryLaplacian(input_dim=120, hidden_dim=32, k=5, use_conv=True, lambda_reg=0.01)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None, burn_history=None):
        ...
        laplacian, burn_graph_loss  = self.burn_history_laplacian(burn_history)
        ...
        out = self.fc_out(torch.cat([out, hidden_features], dim=1))
        # out = self.fc_out(out)
        return out
