...
class SparseDeformableMambaBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x_processed = self.proj_out(torch.stack(outputs, dim=1))
...
class SparseDeformableChannelMambaBlock(nn.Module):
    """沿序列维按输入顺序做 Mamba（DyT + Mamba + 残差）。"""
    def __init__(self, dim, num_heads=13, sparsity_ratio=0.3, drop_rate=0.3):
        super().__init__()
        self.dim = dim
        # num_heads / sparsity_ratio 保留以兼容旧构造调用，已不再使用
        self.num_heads = num_heads
        self.sparsity_ratio = sparsity_ratio
    ...
    def forward(self, x):
        """x: [B, L, C]，按时间/序列下标顺序处理。"""
        residual = x
        x_norm = self.norm(x)
        out = self.drop_path(self.mamba(x_norm)) + residual
        return out
...
class SpatialSpectralMambaBlock(nn.Module):
    def __init__(self, dim: int, patch_size: int, num_heads = 8):
        ...
        self.spatial_mamba = SparseDeformableMambaBlock(dim=dim, drop_rate=0.5, sparsity_ratio=1.0)
        self.spectral_mamba = SparseDeformableChannelMambaBlock(dim=13*13, drop_rate=0.5, sparsity_ratio=1.0)
        self.temporal_mamba = SparseDeformableChannelMambaBlock(dim=13*13, drop_rate=0.5, sparsity_ratio=1.0)
        self.norm = nn.BatchNorm2d(dim)
