...
class UMAPAnchorLaplacian(nn.Module):
    ...
    def forward(self, x):
        ...
        # ==========================================
        # Step 4.5: 强制稀疏化 (Top-K Anchors)
        # ==========================================
        # 物理意义：一个样本最多只能属于 K_anchors 种物理状态原型
        K_anchors = 2  
        
        # 找到每个样本得分最高的 K_anchors 个 Anchor
        topk_vals, topk_inds = torch.topk(S, k=K_anchors, dim=1)
        
        # 创建一个全 0 矩阵，只把 Top-K_anchors 的权重填进去
        ...
        # 可选：将 A 可视化为热力图/网络图（统一使用外部可视化函数）
        if getattr(self, "visualize_adjacency", False):
            visualize_adjacency_heatmap(
                A, save_path="ssm_umap_anchor_A_heatmap.png",
                title="UMAP Anchor Adjacency A (S @ S^T)", annotate=True
            )
            visualize_graph_network(
                A, save_path="ssm_umap_anchor_A_network.png",
                title="UMAP Anchor Graph", max_nodes=64
            )
...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None, burn_history=None):
        ...
        weights = self.gate(hidden_features)  # [B, 2]
        w_burn = weights[:, 0]         # [B]
        w_manifold = weights[:, 1]     # [B]
        W_burn = torch.outer(w_burn, w_burn).sqrt()
        W_manifold = torch.outer(w_manifold, w_manifold).sqrt()
       
        L_fused = W_burn * laplacian + W_manifold * laplacian_manifold
        

        out = self.gcn_stream(hidden_features, L_fused)  # (B, d_model)
