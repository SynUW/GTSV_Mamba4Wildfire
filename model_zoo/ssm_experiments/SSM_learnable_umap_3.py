...
UMAP3相较于UMAP2的区别主要是加上了一个可以学习的A
"""
...
def row_normalize(A: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
    """每行除以行和，将值域统一到 [0, 1] 且行和为 1"""
    row_sum = A.sum(dim=-1, keepdim=True).clamp(min=eps)
    return A / row_sum

class MAAM(nn.Module):
    """
    Multi-channel Adaptive Adjacency Matrix，适配 batch 内样本图（B×B）。
    
    每个通道用独立的 proj1/proj2 将输入特征动态投影为节点嵌入，
    计算样本间相关性后多通道加权融合，最终对称化得到邻接矩阵。
    
    参数：
        input_dim : 输入特征维度（如 d_model=380）
        embed_dim : 节点嵌入维度
        channels  : 通道数（类比多头注意力的头数）
    
    输入：
        x : Tensor [B, input_dim]  每个样本的特征向量
    
    输出：
        A : Tensor [B, B]  归一化后的对称邻接矩阵，值域 [0, 1]
    """
    def __init__(self, input_dim: int, embed_dim: int = 32, channels: int = 4):
        super().__init__()
        self.channels = channels

        # 每个通道独立的投影头
        # proj1 类比 Query，proj2 类比 Key
        # 两者独立（非对称）允许学习有向关系，最终对称化
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.channels):
            E1 = self.proj1[i](x)              # [B, embed_dim]
            E2 = self.proj2[i](x)              # [B, embed_dim]

            # 内积相似度：[B, embed_dim] @ [embed_dim, B] -> [B, B]
            Ai = torch.mm(E1, E2.T)

            # relu 截断负值，softmax 归一化
            Ai = F.softmax(F.relu(Ai), dim=-1) # [B, B]
            A_list.append(Ai)

        # 多通道加权融合
        A_stack = torch.stack(A_list, dim=0)   # [channels, B, B]
        w = F.softmax(self.channel_weights, dim=0)  # [channels] 归一化
        Ae = (w[:, None, None] * A_stack).sum(dim=0)  # [B, B]
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.maam_umap = MAAM(input_dim=380, embed_dim=32, channels=4)
        self.maam_burn = MAAM(input_dim=128, embed_dim=32, channels=4)
        self.fusion_weight_umap = nn.Parameter(torch.tensor(0.0))
        self.fusion_weight_burn = nn.Parameter(torch.tensor(0.0))
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None, burn_history=None):
        ...
        A_maam_umap = self.maam_umap(hidden_features)   # [B, B]
        A_maam_burn = self.maam_burn(burn_history)   # [B, B]
        ...
        alpha_umap = torch.sigmoid(self.fusion_weight_umap)
        alpha_burn = torch.sigmoid(self.fusion_weight_burn)
        
        laplacian_manifold = alpha_umap * laplacian_manifold + (1-alpha_umap) * A_maam_umap
        laplacian = alpha_burn * laplacian + (1-alpha_burn) * A_maam_burn
