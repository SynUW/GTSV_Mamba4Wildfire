...
删除了umap，
并且在最后的输出中，将hidden_features和x_t拼接在一起，之前没有这个拼接
"""
...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None, burn_history=None):
        ...
        x_t = self.PositionalEncoding(doy)  # (B, d_model)
        x_t = x_t * self.PE_Weights
        
        # x_t = x_t.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, H, W)       # broadcast to (B, d_model, H, W)
        ...
        out = self.fc_out(torch.cat([out, hidden_features, x_t], dim=1))
        # out = self.fc_out(out)
        return out
