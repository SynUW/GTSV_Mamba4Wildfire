...
class SpatialSpectralMambaBlock(nn.Module):
    def __init__(self, dim: int, patch_size: int, num_heads = 8):
        ...
        self.spatial_mamba = SparseDeformableMambaBlock(dim=dim, drop_rate=0.5, sparsity_ratio=0.7)
        self.spectral_mamba = SparseDeformableChannelMambaBlock(dim=13*13, drop_rate=0.5, sparsity_ratio=0.7)
        self.temporal_mamba = SparseDeformableChannelMambaBlock(dim=13*13, drop_rate=0.5, sparsity_ratio=0.7)
        self.norm = nn.BatchNorm2d(dim)
...
if __name__ == "__main__":
    ...
    # thop：需 burn_history 走完整 forward；batch=1 时 Laplacian topk 可能 k=0，故用 B=2 估单样本再 /2
    from thop import profile

    class _ModelThopWrap(nn.Module):
        def __init__(self, m: Model):
            super().__init__()
            self.m = m

        def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, burn_history):
            return self.m(x_enc, x_mark_enc, x_dec, x_mark_dec, burn_history=burn_history)

    b_prof = 2
    wrap = _ModelThopWrap(model).eval()
    xe = torch.randn(b_prof, T, C, H, W, device=device)
    xe[:, :, -1, :, :] = torch.randint(1, 18, (b_prof, T, H, W), device=device).float()
    xme = torch.zeros(b_prof, T, mark_f, device=device)
    xme[:, :, -1] = _doy.unsqueeze(0).expand(b_prof, -1)
    xd = torch.zeros(b_prof, pred_len, C, H, W, device=device)
    xmd = torch.zeros(b_prof, pred_len, mark_f, device=device)
    xmd[:, :, -1] = 0.5
    bh = torch.rand(b_prof, H, W, Tb, device=device)

    macs, params_thop = profile(wrap, inputs=(xe, xme, xd, xmd, bh), verbose=False)
    macs_f = float(macs) / b_prof
    flops1 = 2.0 * macs_f
    print(
        f"thop MACs (equiv. batch=1, from B={b_prof}) : {macs_f / 1e6:.2f} MMAC "
        f"({macs_f / 1e9:.3f} GMAC)"
    )
    print(
        f"thop FLOPs (equiv. batch=1): {flops1 / 1e6:.2f} MFLOPs "
        f"({flops1 / 1e9:.3f} GFLOPs, ≈2×MACs)"
    )
    print(
        f"thop 'params'       : {int(params_thop):,} "
        f"(常小于 PyTorch 计数，以 parameters (total) 为准)"
    )
    print(
        f"linear scaled FLOPs (batch={batch_size}, from equiv. batch=1): "
        f"{flops1 * batch_size / 1e9:.3f} GFLOPs"
    )
